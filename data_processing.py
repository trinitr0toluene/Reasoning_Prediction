...
for item in tqdm(islice(dataset, 200000), desc="Processing samples"):
    ...
    # if Dataset == 'glaiveai/reasoning-v1-20m'
    # solution = item.get("response", "").strip()
    # reasoning, solution = extract_think_and_rest(solution)
    
    # if dataset == 'OpenR1-math'

    solution  = (item.get("response (content)") or "").strip()
    ...
    # 统计 general reasoning 的 token 数
    # reasoning_token_count = len(tokenizer.tokenize(reasoning[0]))

    # 统计 openR1 reasoning 的 token 数
...
# 保存为 JSON 文件
with open("openr1math_dataset.json", "w", encoding="utf-8") as f:
